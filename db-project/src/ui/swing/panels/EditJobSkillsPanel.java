...
public class EditJobSkillsPanel extends JPanel {
	 ...
	 * Create the panel.
	 */
	public EditJobSkillsPanel(Connection connection, Job job) {
	...
	private void updateCurrentSkillsListModel() {
		...
		try {
			list = skillQueries.getSkillsForJob(job.getJobCode());
	...
	private void updateAvailableSkillsListModel() {
		List<Skill> list = null;
		try {
			list = skillQueries.getSkillsNotRequiredByJob(job.getJobCode());
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	private class ButtonController implements ActionListener {
		...
		public void addSkills() {
			...
			try {
				skillQueries.addSkillsToJob(job, addList);
			} catch (SQLException e) {
				e.printStackTrace();
			}
			updateModels();
		...
		public void removeSkills() {
			...
			try {
				skillQueries.removeSkillsFromJob(job, removeList);
			} catch (SQLException e) {
				e.printStackTrace();
			}
			updateModels();
